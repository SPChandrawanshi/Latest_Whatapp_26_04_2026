  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-extrabold text-[#0a3d62] tracking-tight">{role} Overview</h2>
          <p className="text-slate-500 mt-1 font-medium italic">Monitor real-time performance & metrics.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="bg-white p-5 rounded-[2rem] shadow-premium border border-transparent hover:border-primary transition-all flex items-center gap-4 group">
            <div className={`w-12 h-12 rounded-2xl ${stat.color} flex items-center justify-center text-white shadow-lg group-hover:rotate-12 transition-transform`}>
              <span className="text-sm font-black uppercase tracking-tighter">{stat.label[0]}</span>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">{stat.label}</p>
              <div className="flex items-center gap-2 mt-1">
                <h3 className="text-xl font-black text-[#0a3d62]">{stat.value}</h3>
                <span className="text-[9px] font-bold text-slate-400">{stat.grow}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 flex flex-col gap-8">
           <div className="bg-white p-8 rounded-[2.5rem] shadow-premium h-80 flex flex-col items-center justify-center border border-slate-50 hover:border-primary transition-all relative overflow-hidden group">
              <div className="text-center z-10">
                <p className="text-slate-300 font-black text-xl uppercase tracking-[0.3em] group-hover:tracking-[0.5em] transition-all">Analytics Canvas</p>
